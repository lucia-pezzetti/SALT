...
parser.add_argument("--num_layers", type=int, default=6, help="Number of layers in the customised grid environment")
...
if args.mode == "random":
    ...
    # seed for reproducibility; you can expose --seed if you like
    rand_key = jax.random.PRNGKey(0)

    def random_policy(state):
        rand_key, subkey = jax.random.split(rand_key)
        # legal actions are those with neighbor_mask==1
        legal_actions = jnp.nonzero(state.neighbor_mask)[0]
        # pick one uniformly at random
        idx = jax.random.randint(subkey, (), 0, legal_actions.shape[0])
    ...
    rl_col = np.argmax(F, axis=1)
    # _, rl_col = linear_sum_assignment(-R)
    ...
    sp_col = np.argmax(emd(a, b, D, numItermax=1000), axis=1)
    # _, sp_col = linear_sum_assignment(D)

...
def build_adj_and_time_matrix(G: nx.DiGraph, max_deg=None, node_to_idx: dict = None):
    ...
    for i, node in enumerate(node_list):
        ...
        for j, nbr in enumerate(neighbors[:max_deg]):
            ...
            time = G[node][nbr][best_k]['travel_time_congested'] * 60  # convert to seconds
            adj[i, j] = node_to_idx[nbr]
            times[i, j] = time

        # pad the rest with the first entry
        if len(neighbors) < max_deg:
            adj[i, len(neighbors):] = adj[i, 0]
            times[i, len(neighbors):] = times[i, 0]
...
def make_obs_fn(G: nx.DiGraph, node_to_idx: dict, max_steps: int):
    ...
    def obs_fn(state_batch):
        return jax.vmap(single_obs)(state_batch)

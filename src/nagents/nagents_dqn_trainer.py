...
from nagents_taxi_env import init_env, TaxiState, TaxiEnv
from nagents_visualization_utils import TrainingLogger
...
def estimate_returns(env: TaxiEnv,
                     params,
                     model: nn.Module,
                     ...
                     gamma: float = 0.99):
        """
        Returns an [N × N] matrix R where R[i,j] ≈ expected discounted return
        starting from (starts[i], pickups[j]) under the current greedy policy.
        """
        N = starts.shape[0]

        # tile out all start–pickup pairs
        s_rep = jnp.repeat(starts[:, None],  N, axis=1).ravel()  # [N*N]
        p_rep = jnp.repeat(pickups[None, :], N, axis=0).ravel()  # [N*N]
        ...
        def body(carry, t):
            st, acc = carry
            obs   = obs_fn_batch(st)
            q     = model.apply(params, obs['state_feats'], obs['action_feats'], st.neighbor_mask, obs['global_feats'])
            act   = jnp.argmax(q, axis=-1)
            nxt, r, _, _ = env.step(st, act)
            acc = acc + (gamma**t) * r
            # nxt, _ = maybe_reset(nxt, None)
            return (nxt, acc), None

        # run for rollout_steps
        (_, total_ret), _ = jax.lax.scan(body,
                                        (states, jnp.zeros(states.current_node.shape[0])),
...
def train(
    ...
    def maybe_reset(
        ...
        def do_reset(_):
            ...
            return resets
        # resets, _ = batched_init(subkeys, init_state.current_node, init_state.pickup_node)            # <--------- FIX THIS!!!!!!!
    ...
    for ep in range(1, epochs+1):        
        ...
        _, col_idx = linear_sum_assignment(-R)    # maximize sum of R
        pickups = fixed_pickups[col_idx]   

        # jax.debug.print("New starts: {}, pickups: {}", new_starts, pickups)  
    ...
    logger.save_plots(out_dir="plots/test")   # writes reward_per_episode.png and avg_wait_per_episode.png

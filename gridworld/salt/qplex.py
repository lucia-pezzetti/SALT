"""
QPLEX-style cooperative MARL baseline (relative-targets only).

Shared per-agent Q-network with a duplex mixing head (softmax weights + state value),
following the spirit of QPLEX (Wang et al.) — not constrained to be monotonic in agent Qs.
"""
...
QPLEX_OBS_MODES = {"relative_targets"}
...
class QPLEXMixer(nn.Module):
    """
    Duplex-style mixing: Q_tot = sum_i alpha_i(s) * Q_i^a_i + V(s),
    with alpha = softmax(W h(s)) (unnormalized logits -> nonnegative weights summing to 1).
    """
    ...
    def forward(self, agent_qs: torch.Tensor, states: torch.Tensor) -> torch.Tensor:
        ...
        B = agent_qs.size(0)
        alpha = F.softmax(self.alpha_head(states), dim=-1)
        v = self.v_head(states).view(B)
        return (alpha * agent_qs).sum(dim=-1) + v


class QPLEXReplayBuffer:
    """Simple ring-buffer replay for transition-level QPLEX updates."""
...
def _validate_obs_mode(obs_mode: str) -> str:
    if obs_mode not in QPLEX_OBS_MODES:
        raise ValueError(
            f"Invalid QPLEX obs mode '{obs_mode}'. "
            f"Expected one of {sorted(QPLEX_OBS_MODES)}."
        )
    return obs_mode
...
def _epsilon_by_episode(ep: int, cfg: QPLEXConfig) -> float:
...
def train_qplex(
    ...
    n_agents: int,
    cfg: QPLEXConfig | None = None,
    seed: int = 0,
    log_every: int = 10,
    print_every: int = 1000,
    n_targets: int | None = None,
    wb_run=None,
    wb_prefix: str = "train_qplex",
) -> Tuple[QMIXQNet, Dict[str, Any]]:
    if cfg is None:
        cfg = QPLEXConfig()
    ...
    q_net = QMIXQNet(obs_dim, n_act, cfg.hidden_size)
    mixer = QPLEXMixer(N, state_dim, cfg.mixer_embed)
    ...
    replay = QPLEXReplayBuffer(cfg.replay_capacity, N, obs_dim, state_dim)
    total_updates = 0
    ...
    for batch_idx in range(cfg.n_batches):
        ...
        s_state = np.empty((H, E, state_dim), dtype=np.float32)
        ...
        s_next_state = np.empty((H, E, state_dim), dtype=np.float32)
        ...
        state_g = np.empty((E, state_dim), dtype=np.float32)
        next_state_g = np.empty((E, state_dim), dtype=np.float32)
        for t in range(H):
            ...
            state_g[:, 0:2*N:2] = pos_r / max(1, grid.h - 1)
            state_g[:, 1:2*N:2] = pos_c / max(1, grid.w - 1)
            state_g[:, 2*N:3*N] = active_f
            state_g[:, 3*N] = t / max(1, H - 1)
            tbase = 3 * N + 1
            state_g[:, tbase:tbase + 2 * M:2] = tgt_r / max(1, grid.h - 1)
            state_g[:, tbase + 1:tbase + 2 * M:2] = tgt_c / max(1, grid.w - 1)
            state_g[:, tbase + 2 * M:] = tgt_active.astype(np.float32)
            ...
            next_state_g[:, 0:2*N:2] = new_r / max(1, grid.h - 1)
            next_state_g[:, 1:2*N:2] = new_c / max(1, grid.w - 1)
            next_state_g[:, 2*N:3*N] = next_active_f
            next_state_g[:, 3*N] = next_t / max(1, H - 1)
            tbase = 3 * N + 1
            next_state_g[:, tbase:tbase + 2 * M:2] = tgt_r / max(1, grid.h - 1)
            next_state_g[:, tbase + 1:tbase + 2 * M:2] = tgt_c / max(1, grid.w - 1)
            next_state_g[:, tbase + 2 * M:] = tgt_active.astype(np.float32)
        ...
        if episodes_done % print_every == 0:
            ...
            print(
                f"[QPLEX-{noise_cfg.kind}|{obs_mode}] batch {batch_idx+1:4d}/{cfg.n_batches}"

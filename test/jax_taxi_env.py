...
class JAXRideEnv(eqx.Module):
    ...
    def step(self, state: TaxiState, action: int) -> tuple[TaxiState, float]:
        ...
        # debug.print("distance current-pickup: {}, distance next-pickup: {}", self.distances[current, state.pickup_node][0], self.distances[next_node, state.pickup_node][0])
        ...
        shaping = jnp.where(dist_next != 0, (dist_current - dist_next) / dist_next, dist_current)

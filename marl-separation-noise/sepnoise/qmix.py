"""
QMIX cooperative MARL baseline (relative-targets only).

Shared per-agent Q-network with a monotonic mixing network:
Q_tot = Mixer(Q_1, ..., Q_N, state).
"""
...
class QMixer(nn.Module):
    def __init__(self, n_agents: int, state_dim: int, embed_dim: int = 32):
        ...
        self.hyper_b2 = nn.Sequential(
            nn.Linear(state_dim, embed_dim), nn.Tanh(), nn.Linear(embed_dim, 1),
        )
...
def _validate_obs_mode(obs_mode: str) -> str:
    if obs_mode not in QMIX_OBS_MODES:
        raise ValueError(
            f"Invalid QMIX obs mode '{obs_mode}'. "
            f"Expected one of {sorted(QMIX_OBS_MODES)}."
        )
    return obs_mode
...
def _epsilon_by_episode(ep: int, cfg: QMIXConfig) -> float:
...
def train_qmix(
    ...
    n_agents: int,
    cfg: QMIXConfig | None = None,
    seed: int = 0,
    log_every: int = 10,
    print_every: int = 1000,
    n_targets: int | None = None,
    wb_run=None,
    wb_prefix: str = "train_qmix",
    ...
    mixer = QMixer(N, state_dim, cfg.mixer_embed)
    ...
    for batch_idx in range(cfg.n_batches):
        ...
        if episodes_done % print_every == 0:
            ...
            print(
                f"[QMIX-{noise_cfg.kind}|{obs_mode}] batch {batch_idx+1:4d}/{cfg.n_batches}"
